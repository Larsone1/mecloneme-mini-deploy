...
def _pick_str(json_key: str, env_aliases: List[str], default: str) -> str:
    if json_key in _MC: return str(_MC[json_key])
...
# ===== Config =====
API_VERSION = _pick_str("api_version", ["MC_API_VERSION","API_VERSION"], "0.3.7")
...
def write_event(kind: str, **fields):
    ...
    try:
        with open(EVENTS_JSONL, "a", encoding="utf-8") as f: f.write(json.dumps(fields, ensure_ascii=False) + "\n")
    except Exception: pass

def write_shadow(**fields):
    fields["ts"] = int(time.time())
    try:
        with open(SHADOW_JSONL, "a", encoding="utf-8") as f: f.write(json.dumps(fields, ensure_ascii=False) + "\n")
    except Exception: pass
...
def _load_progress() -> Dict[str, int]:
    ...
    if not obj:
        obj = PROGRESS_DEFAULT.copy(); _save_json(PROGRESS_PATH, obj)
    return obj
...
def new_session(kid: str):
    ...
    SESSIONS[sid] = {"kid": kid, "exp": exp}; _save_json(SESSIONS_PATH, SESSIONS)
...
async def audit_mw(request: Request, call_next):
    ...
    try:
        ...
        if status >= 500: await emit_alert("crit", "HTTP 5xx", cooldown_s=10, path=path, code=status)
        hot = rate_is_hot(ip)
        if hot is not None: await emit_alert("warn", "Rate hot", cooldown_s=15, ip=ip, count=hot, limit=RATE_MAX)
...
def _compute_summary(n: int) -> Dict[str, Any]:
    ...
    by_status: Dict[str, int] = {}; by_path: Dict[str, int] = {}; by_method: Dict[str, int] = {}; lat: List[int] = []
    for r in rows:
    ...
    return {"statuses": by_status, "paths_top5": sorted(by_path.items(), key=lambda x:x[1], reverse=True)[:5],
            "methods": by_method, "latency_ms": _lat_stats(lat)}
...
def _compute_series(minutes: int = 30) -> Dict[str, Any]:
    ...
    for r in rows:
        t = r.get("ts", now); m = (t // 60) * 60; ms = r.get("ms")
        if isinstance(ms, int): buckets.setdefault(m, []).append(ms)
        counts[m] = counts.get(m, 0) + 1
    series = []
    for m in range((floor_start//60)*60, (now//60)*60 + 60, 60):
        vals = buckets.get(m, []); q = _lat_stats(vals) if vals else {"p50":0,"p95":0,"avg":0,"max":0}
...
def _compute_health(n: int) -> Dict[str, Any]:
    ...
    lat_q = _lat_stats(lat); codes: Dict[str,int] = {}
    for r in rows: codes[str(r.get("status","?"))] = codes.get(str(r.get("status","?")),0)+1
    level = "ok"
    if lat_q["p95"] >= P95_CRIT: level = "crit"
    elif lat_q["p95"] >= P95_WARN: level = "warn"
    return {"ok": True, "ts": int(time.time()), "version": API_VERSION,
            "uptime_s": int(time.time())-BOOT_TS, "req_count": REQ_COUNT,
            "rate_window_s": RATE_WINDOW, "latency_ms": lat_q,
            "codes": codes, "sample_size": len(rows), "level": level,
            "thresholds": {"warn": P95_WARN, "crit": P95_CRIT}}

# ===== Theming =====
def _theme_vars(theme: str) -> Dict[str, str]:
    if theme == "light":
        return {"bg":"#ffffff","tx":"#111","bd":"#e5e5e5","card":"#fafafa","ok":"#2e7d32","warn":"#e09100","crit":"#c62828","btn":"#f3f3f3"}
    return {"bg":"#0b0e11","tx":"#eaeef3","bd":"#1a1f24","card":"#0f1318","ok":"#4ade80","warn":"#fbbf24","crit":"#f87171","btn":"#0f1720"}

# ===== UI renderers (inline SVG Gantt + editor) =====
...
def _http_post_json(url: str, payload: Dict[str, Any], headers: Optional[Dict[str, str]] = None, timeout_s: int = 3) -> Dict[str, Any]:
    ...
    data=_json.dumps(payload).encode(); req=urllib.request.Request(url, data=data, method="POST"); req.add_header("Content-Type","application/json")
    if headers: 
...
def gantt_svg():
    days_total=30; row_h=22; pad_x,pad_y=60,20; width=900; height=pad_y*2 + row_h*len(GANTT_PLAN)
    day_w=(width - pad_x - 20)/days_total; v=_theme_vars(THEME); grid=v['bd']; bar=v['ok']; text=v['tx']
    ...
    for d in range(days_total+1):
        x=pad_x + d*day_w; parts.append(f"<line x1='{x:.1f}' y1='{pad_y-6}' x2='{x:.1f}' y2='{height-pad_y}' opacity='0.5' />")
    parts.append("</g>")
    parts.append(f"<g fill='{text}' font-size='11'>")
    for d in range(0, days_total+1, 5):
        x=pad_x + d*day_w; parts.append(f"<text x='{x:.1f}' y='{pad_y-8}' text-anchor='middle'>{d}d</text>")
    parts.append("</g>")
    y=pad_y; parts.append(f"<g fill='{text}' font-size='12'>")
    for name,off,dur in GANTT_PLAN:
        parts.append(f"<text x='6' y='{y+row_h-6}'>{name}</text>")
        x=pad_x + off*day_w; w=max(1.5, dur*day_w)
...
ALERTS: List[Dict[str, Any]] = []; LAST_ALERT_TS: Dict[str,int] = {}

async def emit_alert(level: str, msg: str, cooldown_s: int = 5, **extra):
    now=int(time.time()); key=f"{level}:{msg}"; last=LAST_ALERT_TS.get(key,0)
    if now - last < cooldown_s: return
    LAST_ALERT_TS[key]=now
    entry=Alert(level=level, msg=msg, ts=now, extra=extra).dict()
    ALERTS.append(entry); ALERTS[:] = ALERTS[-200:]; write_shadow(kind="alert", **entry)

@app.get("/alerts")
def list_alerts(): return {"ok": True, "alerts": ALERTS[-50:]}

# ===== Snapshots / Ingest =====
class Snapshot(BaseModel): tag: str; payload: Dict[str, Any]
@app.post("/snapshots")
def save_snapshot(s: Snapshot):
    name=re.sub(r"[^a-zA-Z0-9_-]", "_", s.tag)[:48]; path=os.path.join(SNAPS_DIR, f"{name}_{int(time.time())}.json")
    with open(path,"w",encoding="utf-8") as f: json.dump({"ts": int(time.time()), "payload": s.payload}, f)
    return {"ok": True, "path": path}

class IngestPayload(BaseModel): tag: Optional[str]="default"; data: Dict[str, Any]
@app.post("/ingest")
def ingest(inp: IngestPayload):
    write_shadow(kind="ingest", tag=inp.tag, data=inp.data); write_event("ingest", tag=inp.tag); return {"ok": True}

# ===== Startup =====
def load_stores():
    global PUBKEYS, SESSIONS
    PUBKEYS=_load_json(PUBKEYS_PATH, {}); SESSIONS=_load_json(SESSIONS_PATH, {})
load_stores()
