...
def save_hist(sid: str) -> None:
    ...
    hp.write_text(
        json.dumps(SESS[sid]["history"], ensure_ascii=False, indent=2), encoding="utf-8"
    )
...
def generate_reply(user_text: str) -> str:
    ...
    raw = [
        f"{role}. Ton: {tone}.",
        "Plan: 1) wybierz mikro-krok, 2) 10 min, 3) wróć z wynikiem",
        f"Fokus: {fokus}",
...
def health():
    return {
        "ok": True,
        "service": "mcm_api",
        "profile": (PROFILE.get("name") or "Superclone"),
    }
...
def export_history(sid: str):
    ...
    return FileResponse(
        path=hp, media_type="application/json", filename=f"{sid}-history.json"
    )
