...
const steps = [
  {
    id: 1,
    title: "Zgody",
    desc: "Nadaj uprawnienia: kamera + mikrofon (demo).",
  },
  {
    id: 2,
    title: "Selfie",
    desc: "Zrób selfie / wybierz plik; zapis w sesji.",
  },
...
export default function App() {
  ...
  const [pitch = 1.0, setPitch] = useState(1.0);
  ...
  useEffect(() => {
    ...
    if (!sid) {
      fetch(`${API}/api/session/new`, { method: "POST" })
        ...
        .then((j) => {
          setSid(j.sid);
          localStorage.setItem("mcm_sid", j.sid);
        });
    }
  }, []);
  ...
  useEffect(() => {
    if (!sid) return;
    fetch(`${API}/api/session/${sid}/history`)
      .then((r) => r.json())
      .then((j) => setMessages(j.history || []))
      .catch(() => {});
    fetch(`${API}/api/files?sid=${sid}`)
      .then((r) => r.json())
      .then(setFiles)
      .catch(() => {});
  }, [sid]);
    ...
    try {
      ...
      mr.onstop = async () => {
        try {
          ...
          setUploadMsg(`✅ ${Math.round(blob.size / 1024)} KB → ${j.url}`);
          setRecState("sent");
          fetch(`${API}/api/files?sid=${sid}`)
            .then((r) => r.json())
            .then(setFiles)
  ...
  const send = async (text) => {
    ...
    try {
      const r = await fetch(`${API}/api/chat/send`, {
        ...
        body: JSON.stringify({ sid, text: msg }),
      });
      const j = await r.json();
  ...
  async function softReset() {
    ...
    const fd = new FormData();
    fd.append("sid", sid);
    await fetch(`${API}/api/session/soft-reset`, {
      method: "POST",
      body: fd,
    }).catch(() => {});
    const j = await (await fetch(`${API}/api/session/${sid}/history`))
      .json()
      .catch(() => ({ history: [] }));
    setMessages(j.history || []);
  }
      ...
      >
        ...
        >
          <div style={{ display: "grid" }}>
            ...
            <small style={{ opacity: 0.75 }}>SID: {sid || "⏳"}</small>
          </div>
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            ...
            >
              <input
                type="checkbox"
                checked={speakOnReply}
                onChange={(e) => setSpeakOnReply(e.target.checked)}
              />{" "}
              Autoodczyt
            </label>
            <div style={{ opacity: 0.85, fontSize: 12 }}>{health}</div>
          </div>
        </div>
        ...
        <div style={{ padding: 24, display: "grid", gap: 14 }}>
          ...
          <p style={{ opacity: 0.85, margin: "0 0 6px 0" }}>{s.desc}</p>
          <>
            {" "}
            {/* Added Fragment */}
            {i === 0 && (
            ...
            {i === 2 && (
              <div style={{ display: "grid", gap: 14 }}>
                ...
                <section style={card}>
                  ...
                  <div style={{ display: "flex", gap: 10 }}>
                    <button style={btn} onClick={() => speak()}>
                      ▶️ Powiedz
                    </button>
                    {!recogOn ? (
                      <button style={btn} onClick={startSTT}>
                        🎤 Start STT
                      </button>
                    ) : (
                      <button
                    ...
                    <button style={btn} onClick={() => setInput(transcript)}>
                      ↗️ Użyj transkrypcji
                    </button>
                    <button
                      style={{
                        ...btn,
                        background: recState === "rec" ? "#b91c1c" : "#1f2937",
                      }}
                  ...
                  <small style={{ opacity: 0.8 }}>{uploadMsg}</small>
                </section>
                ...
                <section style={card}>
                  ...
                  <div style={{ display: "grid", gap: 6 }}>
                    ...
                    <button
                      style={btn}
                      onClick={() => navigator.clipboard?.writeText(API)}
                    >
                      Kopiuj API URL
                    </button>
                    <button style={btn} onClick={downloadExport}>
                      Pobierz transkrypt (.json)
                    </button>
                  </div>
                </section>
              </div>
        ...
        >
          <div style={{ display: "flex", gap: 10 }}>
            ...
            {i < steps.length - 1 ? (
              ...
              <button
                ...
                onClick={() => alert("Superklon: DEMO zakończone ✅")}
              >
                Zakończ
              </button>
            )}
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <button
              ...
              onClick={softReset}
            >
              Reset sesji (zostaw pliki)
            </button>
            <button
              style={{ ...btn, background: "#0e7490" }}
              onClick={() =>
                localStorage.removeItem("mcm_sid") || location.reload()
              }
            >
              Nowa sesja (nowy SID)
            </button>
...
const label = { opacity: 0.85, fontSize: 13 };
const input = {
  background: "#0b1220",
  border: "1px solid #23304a",
  color: "#fff",
